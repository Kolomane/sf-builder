# Name: feats_teamwork_scraper.py
# Purpose: Basic function to webscrape Feats/Teamwork
...
def main():
    ...
    jsonTemplate[f"Teamwork_Feats"] = {}
    print(f"=-=-=-=-=-=-=-=-=-=-=-=\nCLASS = TEAMWORK FEATS")

    thePrint = py_scraper.scraper(f"https://aonsrd.com/Feats.aspx?Category=Teamwork",defaultID)
    print(thePrint)
    ...
    for row in theDinner:
        print(row)
        jsonTemplate["Teamwork_Feats"][f"{row[1]}"] = {
            "SourceURL" : f"{row[0]}",
    ...
    print(output)
    with open('json/feats_teamwork.json', 'w', encoding='utf-8') as f:
